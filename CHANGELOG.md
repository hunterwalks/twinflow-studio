...
- `CODE_OF_CONDUCT.md`：Contributor Covenant v2.1 改编版。
- GitHub Issue 表单（Bug / 功能建议 / 联系入口）与 Pull Request 模板，明确「使用合成数据、不提交真实客户数据、保持 local-first」等边界。
...
### Fixed
- 报告页纳入 Observation 表，观测计数、R016–R024、质量评分与校验页保持一致。
- 项目 JSON 导入恢复并校验 `modelConfig`，避免自定义模型与规则包配置在往返后静默丢失。
- 报告元信息复用 `APP_VERSION`，不再固定输出 v0.6.0。
- Excel/CSV 导入增加 20 MiB 文件上限和明确错误提示。

### Changed
- SheetJS 从 npm registry 的旧版 `xlsx@0.18.5` 升级到官方发行包 `0.20.3`。
- Playwright 改用生产构建启动 E2E，本地单 worker、CI 双 worker；项目水合与下载流程补充稳定等待条件。
- GitHub Actions 安装依赖改为 `npm ci`，截图产物名同步为 `v1.5.1-screenshots`。
- 清理公开构建/点击脚本中的本机绝对路径和工具环境痕迹；README 收敛为面向外部维护者的当前版本说明。

### Quality
- 新增报告四表贯通、模型配置往返、报告版本和导入大小边界回归测试。

## [1.5.0] - 2026-08-24

> 可用性硬化主线（v1.3.0 → v1.5.0）收官：把"辅助说明收敛到帮助页"作为统一原则落地到全部功能页，并完成关系图 / 对比 / 模型 / 帮助 / 项目 / Demo 六页在 v1.4 设计系统上的整体焕新。不新增业务对象、不新增校验规则（保持 24 条）、不改动既有数据流与修复逻辑。

### Added
- **帮助页承载全部功能性说明**：快速开始、隐私与安全、四表数据模型、常见问题（FAQ）集中承载；功能页不再重复"这是什么 / 怎么用"的长段引导。
- 版本号升至 v1.5.0；`src/lib/version.ts` 与 `package.json` 同步更新。

### Changed
- **六页整体焕新（v1.5 设计稿落地）**：`graph / compare / model / help / project / demo` 套用 v1.4 设计系统（Sidebar + TopBar + PageHeader / Card / Button 组件），保留全部业务逻辑、状态与 `data-testid` 契约；关系图保留 hover 高亮、孤立筛选、关系筛选与 SVG 连线。
- **功能页去辅助文字（全局一致）**：移除首页、导入、校验、报告、模型、对比、关系图、项目、Demo 各页的 `lead` 引导句、`note` / `sub` / `muted` 说明段与解释性 `<p>`；说明统一仅由帮助页承载。首页"数据在本地处理"说明卡与版本演进段一并移除（隐私与版本信息见帮助页）。
- 导入向导：移除模板复用、批量自动识别、单表映射、规则引擎结果等区块的引导性说明段，保留 `Stepper` 与全部 `import-*` testid。

### Quality
- 保留全部既有 `data-testid` 契约；typecheck / ESLint（0 warning）/ 单测（233 项全绿）/ `next build`（13 路由全静态预渲染）复核通过。
...
### Fixed
- **关系图节点点击详情失效（P0）**：`graph/page.tsx` 此前未绑定 `onNodeClick`，右侧「对象详情」面板永远为空；现已接入点击 / hover 高亮，并随数据范围自动重置选中。
- **关系图连线几乎不可见（P0）**：父级边颜色由 `#94a3b8` 加深为 `#475569` 并加粗，位于 / 挂载边加宽、放大箭头、增加带背景边标签；hover 节点时高亮其关联边与相邻节点，非关联项淡出。
- **导航栏 `data-testid="nav-home"` 重复（P2）**：Logo 改为 `nav-logo`、「首页」保留 `nav-home`，消除自动化测试严格模式下的重复定位。

### Added
- **关系图按 Space 深度展开布局（P1）**：`lib/graph/layout.ts` 基于 `parentId` 链做 BFS 计算层级深度，Space 列按深度横向展开（园区 → 厂房 → 楼层 → 区域一目了然）；Asset / Sensor 列排布在最右侧空间之后。
- **关系图边方向统一为「父 / 容器 → 子 / 成员」**：父级（space#父 → space#子）、位于（space#空间 → asset#资产）、挂载（asset#资产 → sensor#传感器），箭头语义与中文直觉一致，并支持父级 / 位于 / 挂载三类关系筛选。
- **多表批量导入向导（P1）**：`import/page.tsx` 新增「多表批量导入」卡片——上传含多工作表的 Excel 时，按表名（Space/Asset/Sensor/Observation 或 空间/资产/传感器/观测）自动匹配四表，一键顺序导入并给出整库级合并校验摘要；未识别表可继续走单表模式。

### Changed
- 版本号升至 v1.2.0；`src/lib/version.ts` 与 `package.json` 同步更新。

### Quality
- 全量单测 233 项通过；typecheck 通过（五表布局 / 边方向 / 导入向导均为确定性纯函数，无新增 AI / 后端依赖）。
