...
**当前版本：v1.3.0（导航与首页重构 + 开源贡献指南：分组导航 + 当前页高亮 + 面包屑、状态感知首页、统一空/错态组件、全局轻提示 toast、CONTRIBUTING.md 含 GPT 辅助贡献政策；并继承 v1.2.0 的关系图专项与多表批量导入、v1.1.0 的可配置模型与规则、24 条确定性规则、一键修复、跨项目对比、大表虚拟滚动、分块校验、离线帮助）**
...
- **分组导航栏**（`src/components/NavBar.tsx`）：9 个入口按"数据 / 治理 / 高级"分组，当前页高亮（active 态），窄屏折叠为抽屉，版本号常驻。
