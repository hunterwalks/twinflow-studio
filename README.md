...
**当前版本：v1.4.0（设计系统整体焕新：深色左栏 + 极简顶栏外壳，统一设计令牌与 PageHeader / Card / Button / InfoStrip / Stepper 组件库，首页 / 校验 / 导入 / 报告四页视觉重心明确、信息密度一致；并继承 v1.3.1 导航分组、v1.3.0 面包屑 / 状态感知首页 / 统一空错态 / 全局 toast / CONTRIBUTING GPT 政策，以及 v1.2.0 关系图专项、多表批量导入与 24 条确定性规则等）**
...
## v1.3.0 新增：导航与首页重构 + 开源贡献指南

> 可用性硬化三连第一版（v1.3.0—v1.5.0）：在不新增业务对象、不新增校验规则、不接入外部 AI 运行时的前提下，把前端收口为"进得来、知道下一步、反馈明确"。详细规格见仓库 `02_Context/Spec_Usability_v1.3-v1.5.md`。

- **分组导航栏**（`src/components/NavBar.tsx`）：首页独立常驻；业务入口按"数据建模（导入 / 项目 / 关系图）— 质量治理（校验 / 报告 / 对比）— 高级工具（模型 / 帮助）"三段分组，当前页高亮（active 态），窄屏折叠为抽屉，版本号常驻。
- **面包屑**（`src/components/Breadcrumbs.tsx`）：子页面顶部显示"首页 › 当前页"，首页本身不显示，明确当前位置。
- **状态感知首页**（`src/app/page.tsx`）：空项目态突出「从 Demo 开始 / 导入数据」+ 5 步上手；已载入态显示当前数据集来源、对象计数卡与「继续校验 / 查看关系图 / 导出报告」快捷入口。
- **统一空态 / 错误态**（`EmptyState` / `ErrorState`）：三段式文案（发生了什么 / 为什么 / 下一步），在 `import / validate / graph / report` 复用，风格一致。
- **全局轻提示 toast**（`src/components/Toast.tsx`）：导入成功、Demo 载入、清空项目等关键动作给非阻塞反馈，不依赖 `window.alert`。
