...
**当前版本：v1.1.0（可配置模型与规则：版本化模型配置 + 配置驱动规则包 + `/model` 配置页 + 24 条确定性规则 + 一键修复预览/应用 + 项目元信息 + 跨表检索 + 跨项目对比 + 大表虚拟滚动 + 分块校验 + 离线帮助）**

- 本仓库已落地 v0.1.0–v1.1.0；v1.2+ 标准交换、CLI/CI、可插拔 AI 等能力见 Roadmap，未提前实现
